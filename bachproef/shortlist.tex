...
Het vermindert \textit{boilerplate} code zoals validatie, documentatie, serializatie en versnelt de ontwikkeling van \gls{API}'s met 200-300\%, ook houdt het hierdoor de code leesbaar en onderhoudbaar.
...
Dit waarborgt de schaalbaarheid en onderhoudbaarheid van de applicatie op lange termijn.
...
Om de strikte tijdslimieten te halen en de efficiëntie te verhogen, is er gebruikgemaakt van Celery \autocite{Celery}, een gedistribueerde taskqueue die asynchrone taakverwerking mogelijk maakt. 
...
Ook is hierbij gebruikgemaakt van httpx \autocite{HTTPX} als HTTP-client voor het testen van de API-endpoints, waardoor we realistische tests kunnen uitvoeren die de interactie tussen de client en server simuleren.
