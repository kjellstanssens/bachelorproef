...
van breed gedragen \gls{API}'s maakt efficiënte data-aggregatie complex. 
...
Het doel was om de haalbaarheid hiervan aan te tonen via de ontwikkeling van een \gls{PoC} die ongestructureerde data verzamelt zonder de gebruiker bloot te stellen aan \textit{informatie-overload}.
...
Uit de bevindingen blijkt dat traditioneel scrapen op het moderne web vaak faalt door dynamische content en \textit{anti-bot} mechanismen. 
...
Deze extractie gebeurt met strikte inachtneming van ethische conventies zoals de \gls{AVG} en \texttt{robots.txt}. Om de gebruiker te beschermen tegen \textit{notificatie-overload}, filtert een \textit{matcher} 
deze data op basis van gebruikersvoorkeuren en bundelt de resultaten in maximaal één dagelijkse e-mail.

De conclusie is dat het technisch en architecturaal haalbaar is om de versplinterde Vlaamse concertmarkt via ethisch verantwoorde \textit{webscraping} te centraliseren in één tijdig, gepersonaliseerd systeem. 
